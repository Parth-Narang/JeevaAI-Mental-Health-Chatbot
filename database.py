...
class SentimentVectorDatabase:
    def __init__(self, model_name='all-MiniLM-L6-v2'):
        ...
        """
        try:
            # Try to load the model with offline mode to use cached version if available
            self.encoder = SentenceTransformer(model_name, trust_remote_code=True)
        except Exception as e:
            # If online loading fails, try offline mode with cached models
            try:
                self.encoder = SentenceTransformer(model_name, cache_folder='./models', device='cpu')
            except Exception as e2:
                # Last resort: use a lightweight fallback
                import logging
                logging.warning(f"Could not load SentenceTransformer model: {e2}. Attempting fallback...")
        ...
        self._use_tfidf = getattr(self, '_use_tfidf', False)
